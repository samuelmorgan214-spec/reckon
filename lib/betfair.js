...
export const CERT_LOGIN_HOST = 'identitysso-cert.betfair.com';
...
export function certLogin({ appKey, username, password, cert, key }) {
...
export async function login() {
  ...
  try {
    return await certLogin({ appKey, username, password, cert, key });
  } catch (err) {
    return { ok: false, transportError: String(err.message || err) };
  }
}
